    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            with ctx:
                ...
                outputs = model.compute_loss(input_ids=input_ids, labels=labels, position_ids=position_ids, thought_length=args.thought_length, with_z=args.with_z, num_zs=args.num_zs)
            loss = outputs.loss
            total_loss = loss
            if args.with_z:
            ...
            if step % 100 == 0:
                ...
                ppl = loss.exp().item()
                if args.with_z:
                    ppl_entropy = math.exp(entropy_loss.item())
                    any_correct_accuracy = outputs.any_correct_accuracy.item()
                    avg_correct_accuracy = outputs.avg_correct_accuracy.item()
                    print (f"Step: {step}. PPL: {ppl}. Entropy PPL: {ppl_entropy}. Total Loss: {total_loss.item()}. Token Accuracy: {token_accuracy}. Entropy Loss: {entropy_loss.item()}. Any Correct Acc: {any_correct_accuracy}. Avg Correct Acc: {avg_correct_accuracy}")
                else:
