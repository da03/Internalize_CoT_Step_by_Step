    ...
    for batch in tqdm.tqdm(dataloader):
        ...
        for z_idx in range(num_zs):
            ...
            for i, (input_ids_all_i, beam_output_i) in enumerate(zip(input_ids_all, beam_output)):
                ...
                print (f'Pred   {i}: {pred_text} ({pred_ans_result}) {reason}')
                print ('')
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm.tqdm(train_dataloader):
            ...
            with ctx:
                ...
                outputs = model.compute_loss_grpo(input_ids=input_ids, labels=labels, position_ids=position_ids, thought_length=args.thought_length, with_z=args.with_z, num_zs=args.num_zs, epsilon=args.grpo_epsilon, beta=args.grpo_beta, mu=args.grpo_mu, max_new_tokens=args.max_new_tokens, sample_during_training=args.sample_during_training, is_countdown=args.is_countdown)
            loss = outputs.loss
            total_loss = loss
            #if args.with_z:
