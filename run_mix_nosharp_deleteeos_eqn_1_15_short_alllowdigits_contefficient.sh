...
TOKENIZERS_PARALLELISM=false CUDA_VISIBLE_DEVICES=0 stdbuf -oL -eL python src/train_mix_nosharp_deleteeos_eqn.py \
    --from_pretrained /n/holyscratch01/rush_lab/Users/yuntian/cascade_jun12/train_models/mix_1_to_15_includingzero_padinput_d8_short_nosharp_deleteeos_eqn_val16_alllowdigits/gpt2/checkpoint_35 \
    --remove_start_from 288 \
    --start_epoch_from 36 \
    --model ${MODEL} \
    --data_folder ${FOLDER} \
    --epochs ${EPOCHS} \
