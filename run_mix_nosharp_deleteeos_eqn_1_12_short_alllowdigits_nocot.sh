...
export SAVE=train_models/nocot_mix_${NUM_DIGITS_MIN}_to_${NUM_DIGITS_MAX}_includingzero_padinput_d${REMOVE_PER_EPOCH}_short_nosharp_deleteeos_eqn_val16_alllowdigits/gpt2
mkdir -p $SAVE
TOKENIZERS_PARALLELISM=false CUDA_VISIBLE_DEVICES=0 stdbuf -oL -eL python src/train_mix_nosharp_deleteeos_eqn.py \
    --remove_start_from 999999 \
    --model ${MODEL} \
    --data_folder ${FOLDER} \
    --epochs ${EPOCHS} \
    --lr ${LR} \
