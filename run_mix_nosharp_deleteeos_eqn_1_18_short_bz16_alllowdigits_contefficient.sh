...
TOKENIZERS_PARALLELISM=false CUDA_VISIBLE_DEVICES=0 stdbuf -oL -eL python src/train_mix_nosharp_deleteeos_eqn.py \
    --from_pretrained /n/holyscratch01/rush_lab/Users/yuntian/cascade_jun12/train_models/mix_1_to_18_includingzero_padinput_d8_short_nosharp_deleteeos_eqn_val16_bsz16_acc2_alllowdigits/gpt2/checkpoint_31 \
    --remove_start_from 256 \
    --start_epoch_from 32 \
    --model ${MODEL} \
    --data_folder ${FOLDER} \
    --epochs ${EPOCHS} \
