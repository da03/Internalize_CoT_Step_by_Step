...
export SAVE=train_models/mix_${NUM_DIGITS_MIN}_to_${NUM_DIGITS_MAX}_includingzero_padinput_d${REMOVE_PER_EPOCH}_short_nosharp_deleteeos_eqn_val16_alllowdigits_contefficient_fixpad_latest/gpt2
mkdir -p $SAVE
TOKENIZERS_PARALLELISM=false CUDA_VISIBLE_DEVICES=0 stdbuf -oL -eL python src/train_mix_nosharp_deleteeos_eqn_fixpad.py \
    --from_pretrained /n/holyscratch01/rush_lab/Users/yuntian/cascade_jun12/train_models/mix_1_to_12_includingzero_padinput_d8_short_nosharp_deleteeos_eqn_val16_alllowdigits/gpt2/checkpoint_91 \
    --remove_start_from 735 \
    --start_epoch_from 92 \
    --model ${MODEL} \
    --data_folder ${FOLDER} \
    --epochs ${EPOCHS} \
